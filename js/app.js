    ...
    (() => {
        ...
        let formValidate = {
            ...
            validateInput(formRequiredItem) {
                let error = 0;
                let exp = /\+3\s?[\(]{0,1}9[0-9]{2}[\)]{0,1}\s?\d{3}[-]{0,1}\d{2}[-]{0,1}\d{2}/;
                if ("email" === formRequiredItem.dataset.required) {
                    ...
                    this.addError(formRequiredItem);
                    error++;
                } else if (formRequiredItem.classList.contains("input--phone") && !exp.test(formRequiredItem.value)) {
                    this.addError(formRequiredItem);
                    error++;
        ...
        function initSliders() {
            ...
            if (document.querySelector(".swiper")) {
                new core(".main__slider", {
                    ...
                    navigation: {
                        prevEl: ".main__slide-button-prev",
                        nextEl: ".main__slide-button-next"
                    },
                    on: {}
                });
                new core(".review__slider", {
                    ...
                    loop: true,
                    navigation: {
                        prevEl: ".review__slide-button-prev",
                        nextEl: ".review__slide-button-next"
                    },
                    breakpoints: {
                        830: {
                            slidesPerView: 2
                ...
                new core(".equipment__slider", {
                    ...
                    loop: true,
                    navigation: {
                        prevEl: ".equipment__slide-button-prev",
                        nextEl: ".equipment__slide-button-next"
                    },
                    breakpoints: {
                        420: {
                            slidesPerView: 4
                ...
                function mobileSlider() {
                    if (window.innerWidth <= 520 && "false" == slider.dataset.mobile) {
                        review = new core(slider, {
                            modules: [ Scrollbar ],
                            observer: true,
                            observeParents: true,
                            slidesPerView: 1,
        ...
        const options = document.querySelector(".options-click");
        const customInput = document.querySelector(".customInputContainer");
        const itemInfoTabs = document.querySelector(".description__tabs");
        if (navBtn) document.addEventListener("click", clickEvents);
        function clickEvents(e) {
            const targetElement = e.target;
            if (targetElement.closest(".header__catalog-btn")) {
                navBtn.classList.toggle("_active");
                _slideToggle(navMenu);
            }
            if (targetElement.closest(".mobile-menu__block--catalog")) {
                mobileBtn.closest(".mobile-menu").classList.toggle("_active");
                mobileCatalog.classList.toggle("_active");
                bodyLockToggle(0);
            }
            if (targetElement.closest("[data-parent]")) {
                const itemId = targetElement.closest(".catalog__title").dataset.parent ? targetElement.closest(".catalog__title").dataset.parent : null;
                const item = document.querySelector(`[data-child="${itemId}"]`);
                const catalogNav = document.querySelector(".catalog__navigation--mobile");
                if (item) {
                    targetElement.closest(".catalog__title").classList.toggle("_active");
                    item.classList.toggle("_activeItem");
                    catalogNav.classList.toggle("_hide");
                }
                e.preventDefault();
            }
            if (targetElement.closest(".mobile-menu__back")) {
                const catalogNav = document.querySelector(".catalog__navigation--mobile");
            ...
            if (targetElement.closest(".popular__li")) {
                ...
                targetElement.closest(".popular__ul").querySelectorAll("button").forEach((li => {
            ...
            if (targetElement.closest(".main-catalog__info-btn")) {
                let parent = targetElement.closest(".main-catalog__info-btns");
                parent.querySelectorAll(".main-catalog__info-btn").forEach((el => {
                    el.classList.remove("_active");
                }));
                e.preventDefault();
                targetElement.classList.add("_active");
            }
            if (!options.contains(e.target) && !customInput.contains(e.target)) customInput.classList.remove("show");
        ...
        if (document.documentElement.clientWidth <= 520) {
            ...
            navTabs.setAttribute("data-tabs-animate", 500);
            navTabs.addEventListener("click", (e => {
                if (e.target.classList.contains("_tab-active")) {
                    e.target.classList.remove("_tab-active");
                    _slideUp(e.target.nextSibling);
                }
            }));
        }
        if (itemInfoTabs) if (document.documentElement.clientWidth <= 620) {
            itemInfoTabs.querySelector(".description__nav-title").classList.remove("_tab-active");
            itemInfoTabs.setAttribute("data-tabs-animate", 500);
            itemInfoTabs.addEventListener("click", (e => {
                if (e.target.classList.contains("_tab-active")) {
                    e.target.classList.remove("_tab-active");
                    _slideUp(e.target.nextSibling);
                }
            }));
        } else itemInfoTabs.removeAttribute("data-tabs-animate");
        let fields = document.querySelectorAll(".input--file");
        if (fields) Array.prototype.forEach.call(fields, (function(input) {
            let label = input.nextElementSibling;
            let labelVal = label.querySelector(".popup__file-fake").dataset.value;
